```@meta
CurrentModule = LevinIntegrals
```

# LevinIntegrals.jl

Documentation for [LevinIntegrals.jl](https://github.com/AlokaSahoo/LevinIntegrals.jl).
...
While providing `f` and `g` is convenient, in physics applications particular in Strong-Field Approximation (SFA) [see the [Applications in SFA](applications_sfa.md)] the phase function is an integral and the derivative of the phase function has an algebraic expression. Thus, it is more convenient (also efficient) to provide the derivative of the phase function for the Levin method.
...
For more details on the functions and their keyword arguments, please see the [API Reference](api.md) and [Basic Usage](basic_usage.md) pages.
